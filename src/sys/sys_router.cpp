...
void SysRouter_ExecuteSingle(const String &msg)
{
    ...
    // 13. 拦截：强制触发特异点指令/人物链条 (格式 SPC:ishmael)
    else if (msg.startsWith("SPC:"))
    {
        String spc_id = msg.substring(4);
        spc_id.trim(); // 砍掉多余的空格或回车

        // 【核心修复】：必须加 static，保证指针在异步传递时依然存活！
        static String spc_cache;
        spc_cache = spc_id;

        Evt_SpcForce_t payload = {spc_cache.c_str()};
        SysEvent_Publish(EVT_SPECIAL_FORCE, &payload); // 打包扔给邮局！

        Serial.printf("[路由中心] 收到特异点强行触发指令: ID=%s\n", spc_cache.c_str());
    }
    // 14. 【新增】：手机网页发来的获取具体文本指令 (GET:SPC_TXT:ishmael)
    else if (msg.startsWith("GET:SPC_TXT:")) {
        String req_id = msg.substring(12);
        req_id.trim();
        extern SysSpecials sysSpecials; 
        sysSpecials.syncTextByID(req_id);
    }
}
