/*
【模块职责】系统级运动采样服务。它是正常运行时唯一允许持续读取 LSM6DSV 的模块，负责定频轮询、
最新样本缓存、共享 I2C 故障恢复以及 Light Sleep 前后的传感器功耗切换。
【坐标契约】BSP换算值保留为LSM6DSV传感器坐标；本服务把它统一转换为新板实测机身坐标：
+X向屏幕右、+Y向机身顶部、+Z向屏幕外。正常功能只消费 body_imu，原生坐标只用于采集诊断。
...
/**
 * 一份系统运动样本。
 * 【坐标层级】raw字段是LSM6DSV寄存器整数；sensor_imu是量程换算后的传感器原生坐标；
 * body_imu是固定安装矩阵转换后的机身右手坐标（+X屏幕右、+Y机身顶部、+Z屏幕外）。
 * 【使用约束】产品功能必须显式读取body_imu，不能依赖芯片封装方向；sensor_imu只用于脱线采集、
 * 寄存器换算核对和安装矩阵复核，不能再作为页面或手势的方向语义。
 */
...
/**
 * 初始化 LSM6DSV 和采样状态。当前统一使用 120 Hz、±16 g、±2000 dps；首批双蛇杖
