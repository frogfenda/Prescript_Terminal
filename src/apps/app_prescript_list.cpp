...
private:
        ...
        if (appManager.getLanguage() == LANG_ZH) {
            ...
            for(int i = 0; text[i] != '\0'; ) {
                ...
                if (current_w + cw > max_w || text[i] == '\n') {
                    ...
                    buf_idx = 0; current_w = 0;
                    if (m_actual_lines >= MAX_LINES) break;
                    if (text[i] == '\n') { i++; continue; }
                }
                
                for(int b=0; b<clen; b++) {
                    if (buf_idx < 127) line_buf[buf_idx++] = text[i];
                    i++;
                }
                current_w += cw;
            }
            ...
            while (text[i] != '\0' && m_actual_lines < MAX_LINES) {
                if (text[i] == '\n') {
                    line_buf[buf_idx] = '\0';
                    strncpy(m_lines[m_actual_lines++], line_buf, 128);
                    buf_idx = 0; current_w = 0; i++; continue;
                }
                if (current_w == 0 && text[i] == ' ') { i++; continue; }
                ...
                if (current_w > 0 && current_w + w_len > max_w) {
                    line_buf[buf_idx] = '\0';
                    strncpy(m_lines[m_actual_lines++], line_buf, 128);
                    buf_idx = 0; current_w = 0; continue; 
                }
                ...
                while (i < scan_i) {
                    ...
                    if (current_w + cw > max_w) { 
                        line_buf[buf_idx] = '\0'; 
                        if (m_actual_lines < MAX_LINES) strncpy(m_lines[m_actual_lines++], line_buf, 128);
                        buf_idx = 0; current_w = 0; 
                    }
                    for (int b = 0; b < clen; b++) {
                        if (buf_idx < 127) line_buf[buf_idx++] = text[i];
    ...
    void drawUI() {
        ...
        if (pool->empty()) {
            ...
            if (lang == LANG_ZH) HAL_Screen_ShowChineseLine((sw - ew) / 2, sh / 2 - 12, empty_str);
            else HAL_Screen_ShowTextLine((sw - ew) / 2, sh / 2 - 6, empty_str);
            HAL_Screen_Update();
            return;
        }
