...
class AppAlarmEdit : public AppBase {
    ...
    void drawUI() {
        ...
        if (phase == 2) {
            ...
            char buf[64]; sprintf(buf, zh ? "抹除 [%s] ?" : "DEL [%s] ?", sysConfig.alarms[g_alarm_edit_idx].name.c_str());
            ...
            if (phase == 0) dialAnim.drawNumberDial(28, h, 0, 23, "");
            else dialAnim.drawNumberDial(28, m, 0, 59, "");

            // 4. 操作提示停留在底部：Y坐标 56
            const char* tip;
            if (phase == 0) {
                if (g_alarm_edit_idx >= 0) tip = zh ? "长按删此闹钟 / 单击确认" : "LONG: DELETE / CLICK: NEXT";
...
public:
    ...
    void onKeyShort() override {
        SYS_SOUND_CONFIRM();
        if (phase == 2) { phase = 0; linkAnim.jumpTo(phase); drawUI(); } 
