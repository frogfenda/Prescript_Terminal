...
private:
    ...
    const char *optionText(uint8_t index) const
    {
        ...
        if (index == 0)
            return zh ? "获取纺织机回复" : "ASK THE LOOM";
        return zh ? "吃什么？" : "WHAT TO EAT?";
    }
    ...
    void chooseQuestion()
    {
        ...
        {
            int count = sizeof(kQuestionZh) / sizeof(kQuestionZh[0]);
            m_question = kQuestionZh[random(count)];
        }
        else
        {
            int count = sizeof(kQuestionEn) / sizeof(kQuestionEn[0]);
            m_question = kQuestionEn[random(count)];
        }
    }
