...
public:
    ...
    void onResume() override { 
        for (int i = 0; i < active_coins; i++) coins[i].needs_redraw = true;
        drawStaticUI(); 
        drawActiveCoinsOnly();
    }
...
protected:
    ...
    const char *getItemText(int index) override {
        ...
        if (index == 0) sprintf(text_buf, zh ? "%s运行模式 [ %s ]" : "%sMODE: [ %s ]", cursor, sysConfig.coin_data.mode == 0 ? (zh ? "自动" : "AUTO") : (zh ? "手动" : "MANUAL"));
        else if (index == 1) sprintf(text_buf, zh ? "%s理智波动 [ %+d ]" : "%sSANITY: [ %+d ]", cursor, sysConfig.coin_data.sanity);
        else if (index == 2) sprintf(text_buf, zh ? "%s阵列数量 [ %d ]" : "%sCOINS: [ %d ]", cursor, sysConfig.coin_data.coin_count);
        return text_buf;
    }
    ...
    void onKnob(int delta) override {
        if (is_editing) {
            ...
            if (current_selection == 0) sysConfig.coin_data.mode = sysConfig.coin_data.mode == 0 ? 1 : 0; 
    ...
    void onLongPressed() override {
        if (is_editing) { 
            is_editing = false; SYS_SOUND_CONFIRM(); sysConfig.save(); AppMenuBase::onResume(); 
        } else { 
            SYS_SOUND_NAV(); appManager.popApp(); 
        }
    }
};
