...
private:
    ...
    void drawTacticalSlider(int x, int y, int w, int h, float val, bool is_focus, const char *label, bool is_volume, SystemLang_t lang)
    {
        // 1. 标题居中绘制
        if (is_focus)
        {
        ...
        if (!is_volume)
        {
            // 震动条：显示物理档位
            if (val <= 5.0f)
                snprintf(p_str, sizeof(p_str), "%s", UIStrings::LevelOff(lang));
            else if (val <= 40.0f)
                snprintf(p_str, sizeof(p_str), "%s", UIStrings::LevelMin(lang)); // 1档
            else if (val <= 70.0f)
                snprintf(p_str, sizeof(p_str), "%s", UIStrings::LevelMid(lang)); // 2档
            else
                snprintf(p_str, sizeof(p_str), "%s", UIStrings::LevelMax(lang)); // 3档
        }
        else
        {
            // 音量条：显示真实百分比
...
public:
    ...
    void onLoop() override
    {
        ...
        drawTacticalSlider(UITheme::Volume::LeftX(), UITheme::Volume::SliderY(), UITheme::Volume::SliderW(), UITheme::Volume::SliderH(),
                           display_vol, focus_idx == 0, UIStrings::VolumeLabel(lang), true, lang);
        drawTacticalSlider(UITheme::Volume::RightX(), UITheme::Volume::SliderY(), UITheme::Volume::SliderW(), UITheme::Volume::SliderH(),
                           display_haptic, focus_idx == 1, UIStrings::HapticLabel(lang), false, lang);
