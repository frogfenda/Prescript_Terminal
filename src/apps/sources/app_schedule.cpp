...
{
    return UIStrings::ScheduleTitlePreset(appManager.getLanguage(), idx);
}
const char *Get_Text_Preset(int p_idx)
{
    return UIStrings::ScheduleTextPreset(appManager.getLanguage(), p_idx);
}
...
class AppScheduleEdit : public AppBase
{
    ...
    void drawUI()
    {
        ...
        {
            char buf[64];
            snprintf(buf, sizeof(buf), UIStrings::IsZh(lang) ? "抹除 [%s]?" : "DEL [%s]?", sysConfig.schedules[g_schedule_edit_idx].title.c_str());
            UIFrame::DrawDangerConfirm(UIStrings::DangerTitle(lang),
                                       buf,
                                       UIStrings::ScheduleDeleteConfirmHint(lang));
        }
        else
        {
            // 1. 顶部战术链路区
            // 节点较多，间距设为 95
            linkAnim.draw(UITheme::EditFlow::LinkY(), UIStrings::ScheduleEditStepNames(lang), 6, phase, 95);
            ...
            {
                const char *types[] = {
                    UIStrings::ScheduleTypeName(lang, 0, true),
                    UIStrings::ScheduleTypeName(lang, 1, true),
                    UIStrings::ScheduleTypeName(lang, 2, true),
                    UIStrings::ScheduleTypeName(lang, 3, true)};
                dialAnim.drawStringDial(UITheme::EditFlow::DialY(), t_idx, types, 4);
            }
            else if (phase == 5)
            {
                const char *actions[] = {
                    UIStrings::ScheduleActionName(lang, 0),
                    UIStrings::ScheduleActionName(lang, 1)};
                dialAnim.drawStringDial(UITheme::EditFlow::DialY(), p_idx, actions, 2);
            }

            // 4. 底部状态与操作指引
            bool editing_existing = g_schedule_edit_idx >= 0;
            bool expired = editing_existing && sysConfig.schedules[g_schedule_edit_idx].is_expired;
            UIFrame::DrawTip(UIStrings::ScheduleEditTip(lang, phase, editing_existing, expired));
        }
        HAL_Screen_Update();
    }
...
protected:
    ...
    const char *getTitle() override { return UIStrings::ScheduleExpiredTitle(appManager.getLanguage()); }
    const char *getItemText(int index) override
    {
        SystemLang_t lang = appManager.getLanguage();
        if (index == expired_count)
            return UIStrings::ScheduleBackToList(lang);
        ...
        snprintf(buf, sizeof(buf), "%02d/%02d %02d:%02d %s %s%s", t_info.tm_mon + 1, t_info.tm_mday, t_info.tm_hour, t_info.tm_min, s.title.c_str(), UIStrings::ScheduleExpiredMark(lang), mark);
        return buf;
    }
...
protected:
    ...
    const char *getTitle() override { return UIStrings::ScheduleTitle(appManager.getLanguage()); }
    ...
    const char *getItemText(int index) override
    {
        SystemLang_t lang = appManager.getLanguage();
        if (index == 0)
            return UIStrings::ScheduleOpenExpiredItem(lang);
        if (index == active_count + 1)
            return UIStrings::ScheduleAddItem(lang);
...
void _Cb_SchSync(void *payload)
{
    if (!SysEvent_BleSyncScopeMatches(payload, "SCH"))
        return;
    for (int i = 0; i < sysConfig.schedule_count; i++)
    {
        if (sysConfig.schedules[i].is_expired || sysConfig.schedules[i].is_hidden)
