...
public:
    ...
    static void onSyncReq(void* payload) {
        if (!SysEvent_BleSyncScopeMatches(payload, "COIN"))
            return;
...
protected:
    ...
    {
        return UIStrings::CoinSettingsTitle(appManager.getLanguage());
    }
    ...
    const char *getItemText(int index) override
    {
        ...
        if (index == 0)
            snprintf(text_buf, sizeof(text_buf), "%s%s [ %s ]", cursor, UIStrings::CoinModeLabel(lang), UIStrings::CoinModeValue(lang, sysConfig.coin_data.mode));
        else if (index == 1)
            snprintf(text_buf, sizeof(text_buf), "%s%s [ %+d ]", cursor, UIStrings::CoinSanityLabel(lang), sysConfig.coin_data.sanity);
        else if (index == 2)
            snprintf(text_buf, sizeof(text_buf), "%s%s [ %d ]", cursor, UIStrings::CoinCountLabel(lang), sysConfig.coin_data.coin_count);
        else if (index == 3)
        {
            snprintf(text_buf, sizeof(text_buf), "%s%s [ %s ]", cursor, UIStrings::CoinTypeLabel(lang), UIStrings::CoinMaterial(lang, sysConfig.coin_data.coin_type));
        }
        return text_buf;
    }
    ...
    bool getItemEditParts(int index, const char **prefix, const char **anim_val, const char **suffix) override
    {
        ...
        SystemLang_t lang = appManager.getLanguage();
        static char pref_buf[32];

        if (index == 0)
        {
            snprintf(pref_buf, sizeof(pref_buf), "> %s [ ", UIStrings::CoinModeLabel(lang));
            *prefix = pref_buf;
            *anim_val = UIStrings::CoinModeValue(lang, sysConfig.coin_data.mode);
            *suffix = " ]";
            return true;
        }
        else if (index == 1)
        {
            snprintf(pref_buf, sizeof(pref_buf), "> %s [ ", UIStrings::CoinSanityLabel(lang));
            *prefix = pref_buf;
        ...
        {
            snprintf(pref_buf, sizeof(pref_buf), "> %s [ ", UIStrings::CoinCountLabel(lang));
            *prefix = pref_buf;
        ...
        {
            snprintf(pref_buf, sizeof(pref_buf), "> %s [ ", UIStrings::CoinTypeLabel(lang));
            *prefix = pref_buf;
            *anim_val = UIStrings::CoinMaterial(lang, sysConfig.coin_data.coin_type);
            *suffix = " ]";
            return true;
        }
    ...
    void drawUI() {
        ...
        if (phase == 4) {
            ...
            if (g_coin_edit_idx >= 0) {
                snprintf(buf, sizeof(buf), UIStrings::CoinOverrideFormat(lang), sysConfig.coin_presets[g_coin_edit_idx].name.c_str());
            } else {
                snprintf(buf, sizeof(buf), "%s", UIStrings::CoinSaveAsNew(lang));
            }
            UIFrame::DrawDangerConfirm(UIStrings::CoinOverrideTitle(lang), buf, UIStrings::CoinPresetConfirmHint(lang, g_coin_edit_idx >= 0));
        } else {
            linkAnim.draw(UITheme::EditFlow::LinkY(), UIStrings::CoinPresetEditStepNames(lang), 4, phase, 58);
            ...
            if (phase == 0) dialAnim.drawNumberDial(UITheme::EditFlow::DialY(), bp, 0, 99, "");
                ...
                const char *materials[] = {UIStrings::CoinMaterial(lang, 0), UIStrings::CoinMaterial(lang, 1), UIStrings::CoinMaterial(lang, 2)};
                dialAnim.drawStringDial(UITheme::EditFlow::DialY(), cl, materials, 3);
            }

            UIFrame::DrawTip(UIStrings::CoinPresetEditTip(lang, phase == 0 && g_coin_edit_idx < 0));
        }
        HAL_Screen_Update();
    }
    ...
    void onKeyShort() override {
        ...
        if (phase < 4) {
            ...
            if (g_coin_edit_idx >= 0) {
                ...
                sysConfig.coin_presets[idx].coin_colors = c_str;
                char autoName[16];
                snprintf(autoName, sizeof(autoName), UIStrings::CoinAutoPresetFormat(appManager.getLanguage()), idx + 1);
...
protected:
    ...
    const char *getTitle() override { 
        return UIStrings::CoinMenuTitle(appManager.getLanguage());
    }
    ...
    const char *getItemText(int index) override
    {
        ...
        if (index == 0) 
            return UIStrings::CoinQuickItem(lang);
            
        if (index == sysConfig.coin_preset_count + 1) 
            return UIStrings::CoinNewPresetItem(lang);
            
        if (index == sysConfig.coin_preset_count + 2) 
            return UIStrings::CoinAdvancedItem(lang);
