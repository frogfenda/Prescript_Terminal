...
protected:
    ...
    void drawMenuUI(float v_pos) {
        ...
        if (block_x < min_block_x) block_x = min_block_x; 
        ...
        for (int i = base_idx - 2; i <= base_idx + 2; i++) {
            ...
            if (item_y < -UI_ITEM_SPACING_Y || item_y > sh + UI_ITEM_SPACING_Y) continue;
            ...
            // 防止极致侧弯时字母撞破右侧屏幕边缘
            if (item_x + text_width > sw - 5) item_x = sw - text_width - 5;
